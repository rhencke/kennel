...
let preamble _state _name _comment _used_modules _safe =
  (* Target: Python 3.14t (free-threaded build, no GIL).
     Generated code relies on PEP 649 deferred annotation evaluation (the
     default in 3.14), so forward references in dataclass field annotations
     work without quoting.  The [from __future__ import annotations] below
     is a temporary shim carried forward from the pre-3.14t implementation
     and will be removed by the "clean up generated Python to 3.14t" task. *)
  str "# Generated by rocq-python-extraction (target: Python 3.14t)" ++ fnl () ++
  str "from __future__ import annotations" ++ fnl () ++
