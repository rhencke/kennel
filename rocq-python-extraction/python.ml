...
let preamble _state _name _comment _used_modules _safe =
  ...
  (* [from __future__ import annotations] makes all annotations lazy strings
     (PEP 563).  This means forward references and mutually-recursive types
     work in dataclass field annotations without explicit quoting, and the
     runtime never evaluates them — so no import for the annotated type is
     needed at the point of declaration. *)
