...
let preamble _state _name _comment _used_modules _safe =
  (* Target: Python 3.14t (free-threaded build, no GIL).
     ...
     strings on Python 3.10–3.13, enabling forward references in dataclass field
     types without quoting (e.g. [class EvenS: arg0: Odd] before [Odd] is
     defined).  On Python 3.14 this import is a harmless no-op because PEP 649
     deferred annotation evaluation is the default. *)
