...
<!--
  ...
  Input:  structural XML in xmlns="https://fidocancode.dog/fido"
  ...
  Root <fido> element layout:
    <fido_uptime_seconds>  — seconds since fido started
...
<xsl:stylesheet version="1.0"
  ...
  xmlns:k="https://fidocancode.dog/fido"
...
<xsl:template match="/">
  ...
  <dashboard>
    ...
    <!-- Dashboard header: fido uptime + GitHub rate limits -->
    <xsl:if test="k:fido/k:fido_uptime_seconds != '' or k:fido/k:rate_limit/k:rest/k:used != ''">
      <header>
        <xsl:if test="k:fido/k:fido_uptime_seconds != ''">
          <fido-uptime>
            <xsl:text>up </xsl:text>
            <xsl:call-template name="format-duration">
              <xsl:with-param name="seconds" select="k:fido/k:fido_uptime_seconds"/>
            </xsl:call-template>
          </fido-uptime>
        </xsl:if>
        <xsl:if test="k:fido/k:rate_limit/k:rest/k:used != ''">
          <rate-limits>
            <rate-window dog:kind="rest">
              <rate-label>REST</rate-label>
              <rate-nums>
                <xsl:value-of select="k:fido/k:rate_limit/k:rest/k:used"/>
                <xsl:text>/</xsl:text>
                <xsl:value-of select="k:fido/k:rate_limit/k:rest/k:limit"/>
              </rate-nums>
              <xsl:if test="k:fido/k:rate_limit/k:rest/k:resets_at != ''">
                <rate-reset>
                  <xsl:text>resets </xsl:text>
                  <xsl:call-template name="format-time">
                    <xsl:with-param name="iso" select="k:fido/k:rate_limit/k:rest/k:resets_at"/>
                  </xsl:call-template>
                </rate-reset>
              </xsl:if>
            </rate-window>
            <xsl:if test="k:fido/k:rate_limit/k:graphql/k:used != ''">
              <rate-window dog:kind="graphql">
                <rate-label>GraphQL</rate-label>
                <rate-nums>
                  <xsl:value-of select="k:fido/k:rate_limit/k:graphql/k:used"/>
                  <xsl:text>/</xsl:text>
                  <xsl:value-of select="k:fido/k:rate_limit/k:graphql/k:limit"/>
                </rate-nums>
                <xsl:if test="k:fido/k:rate_limit/k:graphql/k:resets_at != ''">
                  <rate-reset>
                    <xsl:text>resets </xsl:text>
                    <xsl:call-template name="format-time">
                      <xsl:with-param name="iso" select="k:fido/k:rate_limit/k:graphql/k:resets_at"/>
                    </xsl:call-template>
                  </rate-reset>
                </xsl:if>
              </rate-window>
    ...
    <xsl:choose>
      <xsl:when test="k:fido/k:repo">
        <xsl:apply-templates select="k:fido/k:repo"/>
      </xsl:when>
      <xsl:otherwise>
