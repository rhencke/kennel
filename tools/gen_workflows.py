...
# Cancel an in-flight run when a newer commit lands on the same PR or
# branch. Pushes to ``main`` are exempted: ``cancel-in-progress`` only
# fires for non-main events, so a merge to main always finishes.
concurrency:
