...
CI_GENERATOR = REPO / ".github" / "workflows" / "generate_ci.py"
...
class TestModelsBuildScript:
    ...
    def test_passes_cache_context_and_local_output(self) -> None:
        ...
        assert '--output "type=local,dest=$repo_root"' in script
        assert '--smart-output "$repo_root/src/fido/rocq"' in script
        assert '--file "$repo_root/rocq-python-extraction/Dockerfile"' in script
    ...
    def test_explicit_check_targets_skip_local_output_export(self) -> None:
        ...
        assert "type=local" not in ci_branch
    ...
    def test_ci_workflow_is_generated_with_per_target_caches(self) -> None:
        ...
        assert "Generated by .github/workflows/generate_ci.py" in workflow
        assert "Buildx gha cache scopes discovered from bake:" in workflow
